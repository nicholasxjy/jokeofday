    ...
    User.getUserById(req.session.user._id, function(err, user) {
        ...
        if (canvas_base64){
            ...
            ndir.mkdir(picDir, function(err) {
                ...
                var qiniuUrl = '/upload_pics/pictures/' + dateStamp + '/' + user.name + '/' + filename;
                pictures.push(config.qiniudomain + qiniuUrl);
                fs.writeFile(savepath, binaryData, 'binary', function(err) {
                    if (err) {
                        return next(err);
                    }
                    //add qi niu store
                    config.qnClient.uploadFile(savepath, {key: qiniuUrl}, function(err, result) {
                        if (err) {
                            return next(err);
                        }
                        rimraf(path.resolve(config.rmupload_dir), function(err) {
                            if (err) {
                                return next(err);
                            }
                        });
